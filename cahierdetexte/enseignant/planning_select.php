...
$query_res = sprintf("SELECT DISTINCT nom_classe,ID_classe FROM cdt_classe,cdt_emploi_du_temps WHERE cdt_classe.ID_classe=cdt_emploi_du_temps.classe_ID AND prof_ID=%u ORDER BY nom_classe ASC",$_SESSION['ID_prof']);
...
<?php   while($row = mysqli_fetch_assoc($res)){
        echo "<option style='text-indent: 30px;' value='".$row["ID_classe"]."'";
