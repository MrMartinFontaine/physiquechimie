...
if (isset($_POST["sup_ok"]))
{
	if ((isset($_POST['ID_fichiers'])) && ($_POST['ID_fichiers'] != "")) {
		...
		//On supprime le fichier uniquement 
		//si une seule entree dans cdt_joint vers ce fichier (il n'y pas eu de copier coller)
		//ou multiple entree mais la duplication est due a un regroupement
		if (($totalRows_Recordset3==1)||($_GET['gic_ID']>0)){
		...
		if ($_GET['gic_ID']>0){ //regroupement
			$deleteSQL = sprintf("DELETE FROM cdt_fichiers_joints WHERE nom_fichier=%s",
			GetSQLValueString($row_Recordset3['nom_fichier'], "text"));
		}
		else {
			$deleteSQL = sprintf("DELETE FROM cdt_fichiers_joints WHERE ID_fichiers=%u",
...
if (isset($_GET['nom_fichier'])) {
	$exp = "/^[0-9]+_/"; $nom_f = preg_replace($exp, '', $_GET['nom_fichier']); ?>
	...
	<tr>
	<th width="50%" valign="top" scope="col"><div align="right">
	<p>
	<input name="Annuler" type="submit" id="Annuler7" onClick="MM_goToURL('window','ecrire.php<?php echo $lien ?>');return document.MM_returnValue" value="Annuler">
        ...
        <th width="50%" valign="top" scope="col"> <form name="form1" method="post" action="fichier_supprime.php<?php echo $lien ?>">
        <div align="left">
        <p>
...
<?php } else { ?>
        Aucun fichier n'a &eacute;t&eacute; s&eacute;lectionn&eacute;. Veuillez recommencer.
<?php };
