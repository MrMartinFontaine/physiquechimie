...
if (!defined('__CLASS_PARSINGHTML__'))
{
	...
	class parsingHTML
	{
		...
		var $code	= array();		// code HTML parse
		var $num	= 0;			// numero de table
...
//						}
			...
			// pour chaque action identifiee, il faut nettoyer le debut et la fin des textes
			...
			for($k=0; $k<$nb; $k++)
			{
				...
				{
					// et qu'une balise specifique le precede => on nettoye les espaces du debut du texte
					if ($k>0 && in_array($todos[$k-1]['name'], $balises_clean))
						$todos[$k]['param']['txt'] = ltrim($todos[$k]['param']['txt']);

					// et qu'une balise specifique le suit => on nettoye les espaces de la fin du texte
		 ...
		 * parser le code HTML
		 *
		 * @param	&array	tableau de retour des donnees
		 * @return	null
		 */
		function searchCode(&$tmp)
		{
			...
			while(preg_match($reg, $this->html, $parse, PREG_OFFSET_CAPTURE, $offset))
			{
				...
				{
					// ajout du texte à la fin de celui qui est dejà detecte
					$str.= $parse[2][0];
				}
			...
			// si un texte est present à la fin, on l'enregistre
			if ($str!='') $tmp[] = array('txt',$str);
			unset($str);
		}
		...
		function analiseCode($code)
		{
			...
			if (isset($match[1])) {	$close	= ($match[1]=='/' ? true : false); };
			$autoclose = preg_match('/\/>$/isU', $code);
			if (isset($match[2])) {$name = strtolower($match[2]);};
			
			// parametres obligatoires en fonction du nom de la balise
			$param	= array();
			$param['style'] = '';
			if ($name=='img')	{ $param['alt'] = '';	$param['src'] = ''; }
			if ($name=='a')		{ $param['href'] = '';	}
			
			// lecture des parametres du type nom=valeur
			$prop = '([a-zA-Z0-9_]+)=([^"\'\s>]+)';
			...
			// mise en conformite en style de chaque parametre
			$color	= "#000000";
			$border	= null;
			...
			// lecture des styles - decomposition
			...
			// determination du niveau de table pour les ouverture, avec ajout d'un level
			...
			// attribution du niveau de table où se trouve l'element
			...
			// retour de l'action identifiee
			return array('name' => $name, 'close' => $close ? 1 : 0, 'autoclose' => $autoclose, 'param' => $param);
		}
		
		// recuperer un niveau complet d'HTML entre une ouverture de balise et la fermeture correspondante
		function getLevel($k)
		{
			// si le code n'existe pas : fin
			if (!isset($this->code[$k])) return '';
			
			// quelle balise faudra-t-il detecter
