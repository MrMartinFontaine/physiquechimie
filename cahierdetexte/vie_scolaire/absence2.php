...
<td> 
<form name="frm" method="GET" action="absence2.php">
<script>
$(function() {
...
<p align="center"><img src="../images/lightbulb.png" width="16" height="16"> S&eacute;lectionner la p&eacute;riode voulue puis  une classe parmi 
  celles dans lesquelles au moins un &eacute;l&egrave;ve a &eacute;t&eacute; not&eacute; absent.</p>
<p align="center" class="Style13">
  Absences et retards pour la p&eacute;riode du &nbsp;
...
if ((isset($_GET['classe']))&&($_GET['classe']!='-1')){   
	...
	$query_Rsabsent = sprintf("
		SELECT nom_ele,prenom_ele,classe,count(eleve_ID),retard,eleve_ID FROM ele_absent,ele_liste  
		WHERE 
	...
	if ($totalRows_Rsabsent>0){
		...
		<td class="Style6" ><div align="left">Nb d'absences en cours &nbsp;</div></td>
		<td class="Style6" ><div align="left">Nb de retards en cours &nbsp;</div></td>
		...
		do {
			...
			$query_Rsretard = sprintf("
				...
				AND SUBSTRING(ele_absent.code_date,1,8)<='%s'
				AND SUBSTRING(ele_absent.code_date,1,8)>='%s'
				AND retard='O'",$row_Rsabsent['eleve_ID'],$_GET['classe'],$datemaxi,$datemini);
			
			$Rsretard = mysqli_query($conn_cahier_de_texte, $query_Rsretard) or die(mysqli_error($conn_cahier_de_texte));
			$row_Rsretard = mysqli_fetch_assoc($Rsretard);
			$totalRows_Rsretard = mysqli_num_rows($Rsretard);
			...
			//enlever les retards
			$abs=$row_Rsabsent['count(eleve_ID)']-$row_Rsretard['count(eleve_ID)'];
			if($alterne>0){echo '<td  class="tab_detail_gris">';}else {
			echo '<td  class="tab_detail_rose">';};
			echo  '&nbsp;&nbsp;&nbsp;'.$abs. '</td>';
			
			
			if($alterne>0){echo '<td  class="tab_detail_gris">';}else {echo '<td  class="tab_detail_rose">';};
			echo '&nbsp;&nbsp;&nbsp;'.$row_Rsretard['count(eleve_ID)']. '</td>';
			$alterne=$alterne*(-1);
		...
		echo "<p>&nbsp;</p><p align=\"center\"> Pas d'absents sur cette p&eacute;riode.</p>";
	};
